...
public final class AbstractCrudServiceTest {
    ...
    public void initializeService() {
        this.service = new AbstractCrudService<>(this.mockedMapper, this.mockedRepository) {
        };
    }
    ...
    public void messagesShouldBeSaved() {
        ...
        when(this.mockedMapper.toEntity(anyCollectionOf(Message.class))).thenReturn(givenMessageEntitiesToBeSaved);
        ...
        when(this.mockedMapper.toDto(anyCollectionOf(MessageEntity.class))).thenReturn(givenSavedMessages);

        final List<Message> actual = this.service.saveAll(givenMessagesToBeSaved);
        assertSame(givenSavedMessages, actual);

        verify(this.mockedMapper, times(1)).toEntity(this.messagesArgumentCaptor.capture());
        verify(this.mockedRepository, times(1))
                .saveAll(this.messageEntitiesArgumentCaptor.capture());
        verify(this.mockedMapper, times(1)).toDto(this.messageEntitiesArgumentCaptor.capture());
